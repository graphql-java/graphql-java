...
public class Traverser<T> {
    ...
    private final ConcurrentHashMap<Class<?>, Object> rootVars = new ConcurrentHashMap<>();

    private static final List<TraversalControl> CONTINUE_OR_QUIT = List.of(CONTINUE, QUIT);
    ...
    private static <T> Function<? super T, Map<String, ? extends List<T>>> wrapListFunction(Function<? super T, ? extends List<T>> listFn) {
        return node -> {
            ...
            return LinkedHashMap.of(null, childs);
        };
    }

    public Traverser<T> rootVars(Map<Class<?>, Object> rootVars) {
        this.rootVars.putAll(assertNotNull(rootVars).toJavaMap());
        return this;
    }
    ...
    public TraverserResult traverse(Collection<? extends T> roots, TraverserVisitor<? super T> visitor) {
        ...
        while (!traverserState.isEmpty()) {
            ...
            if (top instanceof TraverserState.EndList) {
                java.util.Map<String, java.util.List<TraverserContext<T>>> childrenContextMap = ((TraverserState.EndList<T>) top).childrenContextMap;
