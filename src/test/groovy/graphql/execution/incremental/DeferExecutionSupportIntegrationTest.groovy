    ...
    void setup() {
        def runtimeWiring = RuntimeWiring.newRuntimeWiring()
                ...
                .type(newTypeWiring("Post")
                        .dataFetcher("summary", resolve("A summary", 10))
                        .dataFetcher("text", resolve("The full text", 100))
                        .dataFetcher("wordCount", resolve(45999, 10, true))
                        .dataFetcher("latestComment", resolve([title: "Comment title"], 10))
                        .dataFetcher("dataFetcherError", resolveWithException())
                        .dataFetcher("dataAndError", resolveWithDataAndError("data"))
                        .dataFetcher("coercionError", resolve("Not a number", 10))
                        .dataFetcher("typeMismatchError", resolve([a: "A Map instead of a List"], 10))
                        .dataFetcher("nonNullableError", resolve(null))
                )
                .type(newTypeWiring("Page")
                        .dataFetcher("summary", resolve("A page summary", 10))
                        .dataFetcher("text", resolve("The page full text", 100))
                )
                .type(newTypeWiring("Comment")
                        .dataFetcher("content", resolve("Full content", 100))
                        .dataFetcher("author", resolve([name: "Author name"], 10))
                )
