...
class AstPrinterV2Test extends Specification {
    ...
    String printAst(String input) {
        ...
        AstPrinterV2.printAst(document)
    }

    String printAst(Node node) {
        AstPrinterV2.printAst(node)
    }
...
'''
        def document = parse(query)
        String output = AstPrinterV2.printAstCompact(document)
...
'''
        def document = parse(query)
        String outputCompact = AstPrinterV2.printAstCompact(document)
        String outputFull = AstPrinterV2.printAst(document)
...
'''
    ...
    def 'StringValue is converted to valid Strings'() {
        ...
        when:
        def result = AstPrinterV2.printAstCompact(new StringValue(strValue))
    ...
    def 'Interfaces implementing interfaces'() {
        ...
        when:
        def result = AstPrinterV2.printAstCompact(interfaceType)
    ...
    def "directive definitions can be printed"() {
        ...
        when:
        def result = AstPrinterV2.printAstCompact(directiveDef1)

        then:
        result == "directive @d1 repeatable on FIELD | OBJECT"

        when:
        result = AstPrinterV2.printAstCompact(directiveDef2)
