...
class ArgumentsOfCorrectTypeTest extends ValidationRuleTest {
    ...
    def "invalid type results in error"() {
        ...
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg' with value 'StringValue{value='string'}' is not a valid 'Boolean'"
    }
    ...
    def "invalid input object type results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg.foo' with value 'StringValue{value='string'}' is not a valid 'Boolean'"
    }
    ...
    def "invalid list object type results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg[1].foo' with value 'StringValue{value='string'}' is not a valid 'Boolean'"
    }
    ...
    def "invalid list inside object type results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg[0].foo[1]' with value 'StringValue{value='string'}' is not a valid 'Boolean'"
    }
    ...
    def "invalid list simple type results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg[1]' with value 'StringValue{value='string'}' is not a valid 'Boolean'"
    }
    ...
    def "type missing fields results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg' with value 'ObjectValue{objectFields=[ObjectField{name='foo', value=StringValue{value='string'}}]}' is missing required fields '[bar]'"
    }
    ...
    def "type not object results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg' with value 'StringValue{value='string'}' must be an object type"
    }
    ...
    def "type null fields results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg.bar' with value 'NullValue{}' must not be null"
    }
    ...
    def "type with extra fields results in error"() {
        ...
        errorCollector.containsValidationError(ValidationErrorType.WrongType)
        errorCollector.errors.size() == 1
        errorCollector.errors[0].message == "Validation error (WrongType) : argument 'arg' with value 'ObjectValue{objectFields=[ObjectField{name='foo', value=StringValue{value='string'}}, ObjectField{name='bar', value=StringValue{value='string'}}, ObjectField{name='fooBar', value=BooleanValue{value=true}}]}' contains a field not in 'ArgumentObjectType': 'fooBar'"
    }
