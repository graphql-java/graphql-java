...
class VariablesNotAllowedInConstantDirectivesTest extends Specification {
    ...
    def schema = TestUtil.schema('''
        ...
        directive @strDir(arg: String) on VARIABLE_DEFINITION | FIELD | QUERY
        directive @inputDir(arg: InputArg) on VARIABLE_DEFINITION
        input InputArg {
            value: Int
            nested: InputArg
            values: [Int]
    ...
    def "variable reference in operation directive argument is accepted"() {
        given:
        def query = '''
            query ($v: String) @strDir(arg: $v) { x }
        '''
    ...
    def "nested variable reference in directive argument on variable definition is rejected"() {
        given:
        def query = '''
            query ($v: Int, $w: Int @inputDir(arg: {nested: {values: [1, $v]}})) { x }
        '''
