...
class TraversalContextTest extends Specification {
    ...
    def "operation definition"() {
        ...
        when:
        traversalContext.enter(operationDefinition, [], newEnvironment().build())

        then:
        traversalContext.getOutputType() == queryType

        when:
        traversalContext.leave(operationDefinition, [], newEnvironment().build())
    ...
    def "SelectionSet saves current output type as parent"() {
        ...
        when:
        traversalContext.enter(selectionSet, [], newEnvironment().build())

        then:
        traversalContext.getParentType() == droidType

        when:
        traversalContext.leave(selectionSet, [], newEnvironment().build())
    ...
    def "field saves output type and fieldDefinition"() {
        ...
        when:
        traversalContext.enter(field, [], newEnvironment().build())
        ...
        when:
        traversalContext.leave(field, [], newEnvironment().build())
    ...
    def "directives are saved"() {
        ...
        when:
        traversalContext.enter(directive, [], newEnvironment().build())

        then:
        traversalContext.getDirective() == Directives.SkipDirective

        when:
        traversalContext.leave(directive, [], newEnvironment().build())
    ...
    def "inlineFragment type condition saved as output type"() {
        ...
        when:
        traversalContext.enter(inlineFragment, [], newEnvironment().build())

        then:
        traversalContext.getOutputType() == droidType

        when:
        traversalContext.leave(inlineFragment, [], newEnvironment().build())
    ...
    def "fragmentDefinition type condition saved as output type"() {
        ...
        when:
        traversalContext.enter(fragmentDefinition, [], newEnvironment().build())

        then:
        traversalContext.getOutputType() == droidType

        when:
        traversalContext.leave(fragmentDefinition, [], newEnvironment().build())
    ...
    def "variableDefinition saved as input type"() {
        ...
        when:
        traversalContext.enter(variableDefinition, [], newEnvironment().build())

        then:
        traversalContext.getInputType() == GraphQLString

        when:
        traversalContext.leave(variableDefinition, [], newEnvironment().build())
    ...
    def "field argument saves argument and input type"() {
        ...
        when:
        traversalContext.enter(argument, [], newEnvironment().build())
        ...
        when:
        traversalContext.leave(argument, [], newEnvironment().build())
    ...
    def "array value saves input type"() {
        ...
        when:
        traversalContext.enter(arrayValue, [], newEnvironment().build())

        then:
        traversalContext.getInputType() == GraphQLString

        when:
        traversalContext.leave(arrayValue, [], newEnvironment().build())
    ...
    def "object field saves input type"() {
        ...
        when:
        traversalContext.enter(objectField, [], newEnvironment().build())

        then:
        traversalContext.getInputType() == GraphQLString

        when:
        traversalContext.leave(objectField, [], newEnvironment().build())
