...
internal interface DocumentGenEnv {
    ...
    val inputValueGenerator: GraphQLInputValueGen
    ...
    private data class Impl(
        ...
        override val inputValueGenerator = GraphQLInputValueGen(
    ...
    companion object {
        operator fun invoke(schema: GraphQLSchema, cfg: Config, rs: RandomSource): DocumentGenEnv =
            this(Schemas(schema), cfg, rs)

        operator fun invoke(schemas: Schemas, cfg: Config, rs: RandomSource): DocumentGenEnv =
            Impl(schemas, cfg, rs)
    }
}
    ...
    private fun genVariable(
        ...
        val default =
            if (rs.sampleWeight(cfg[DefaultValueWeight])) {
                inputValueGenerator.gen(forType)
            } else {
                null
            }
