...
fun Arb.Companion.graphQLSchema(
    ...
    arbitrary { rs ->
        GraphQLSchemaGen(cfg, rs).gen(types)
    }

/** Methods for generating GraphQLSchema instances */
internal class GraphQLSchemaGen(val cfg: Config, val rs: RandomSource) {
    ...
    fun gen(types: GraphQLTypes): GraphQLSchema {
    ...
    private fun finalize(schema: GraphQLSchema): GraphQLSchema {
    ...
    private fun inputValueGenerator(schema: GraphQLSchema): GraphQLInputValueGen =
        GraphQLInputValueGen(
            schema = schema,
            config = cfg,
            randomSource = rs,
...
internal class AddDefaults(
    private val inputValueGenerator: GraphQLInputValueGen,
    ...
    private fun genDefaultValue(type: GraphQLInputType): Value<*> {
        return inputValueGenerator.gen(type)
    }
}
                    ...
                    .transform {
                        val args = dir.arguments.map { arg ->
                            arg.toAppliedArgument().transform { b ->
                                b.valueLiteral(inputValueGenerator.gen(arg.type))
