...
class GraphQLRuntimeWiringsTest : ArbPropertyBase() {
    @Test
    fun IDValueGenFactory() {
        val factory = IDValueGen.Factory { IDValueGen { "configured-id" } }
        val config = Config(IDValueGenFactory(factory))
        val result = arbGraphQL(
            schema = "type Query { id: ID! }".asSchema,
            seed = 123L,
            config = config
        ).execute("{ id }")

        assertTrue(result.errors.isEmpty())
        assertEquals("configured-id", result.getData<Map<String, Any?>>()?.get("id"))
    }

    @Test
    fun `arbCodeRegistry registers data fetchers`() {
        val schema = "type Query { value: Int! }".asSchema
        val field = schema.queryType.getFieldDefinition("value")
        val coordinates = FieldCoordinates.coordinates("Query", "value")

        val original = schema.codeRegistry.getDataFetcher(coordinates, field)
        val arbitrary = arbCodeRegistry(schema, seed = 123L).getDataFetcher(coordinates, field)

        assertNotSame(original, arbitrary)
    }

    @Test
    fun `arbGraphQL creates an executable instance`() {
        val result = arbGraphQL(
            schema = "type Query { value: Int! }".asSchema,
            seed = 123L
        ).execute("{ value }")

        assertTrue(result.errors.isEmpty())
        assertTrue(result.getData<Map<String, Any?>>()?.get("value") is Int)
    }
    ...
    fun `returns different data for the same query and different seeds`(): Unit =
        runBlocking {
            ...
            val cfg = Config(AppliedDirectiveWeight(CompoundingWeight.Never))
    ...
    fun NullNonNullableWeight(): Unit =
        runBlocking {
            ...
            // disabled
            Config(NullNonNullableWeight(0.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
            ...
            // enabled
            Config(NullNonNullableWeight(1.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
    ...
    fun ExplicitNullValueWeight(): Unit =
        runBlocking {
            ...
            // disabled
            Config(ExplicitNullValueWeight(0.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
            ...
            // enabled
            Config(ExplicitNullValueWeight(1.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
    ...
    fun ResolverExceptionWeight(): Unit =
        runBlocking {
            ...
            // disabled
            Config(ResolverExceptionWeight(0.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
            ...
            // enabled
            Config(ResolverExceptionWeight(1.0)).let { cfg ->
                Arb
                    .long()
                    .map { seed -> mkGraphQL(sdl, arbRuntimeWiring(sdl, seed, cfg)) }
                    .flatMap { gql ->
