...
class GraphQLOutputValuesTest : FunSpec({
    test("nullable and non-null scalar values honor ExplicitNullValueWeight") {
        ...
        checkAll(PropTestConfig(iterations = 20), Arb.graphQLOutputValue(schema, scalar, null, config = nullConfig)) {
            it.shouldBe(null)
        }
        val nonNull = GraphQLNonNull.nonNull(scalar)
        checkAll(PropTestConfig(iterations = 20), Arb.graphQLOutputValue(schema, nonNull, null, config = nullConfig)) {
            it.shouldBeInstanceOf<Int>()
        }
    }

    test("list wrappers preserve configured size and element nullability") {
        val schema = parseTestSchema("type Query { x: Int }")
        val scalar = schema.getType("Int") as GraphQLOutputType
        ...
        checkAll(PropTestConfig(iterations = 20), Arb.graphQLOutputValue(schema, type, null, config = config)) { value ->
    ...
    test("objects honor aliases, inline fragments, and named fragments") {
        ...
        checkAll(PropTestConfig(iterations = 30), Arb.graphQLOutputValue(schema, document, config)) { root ->
    ...
    test("SelectedTypeBias chooses a concrete type selected by a fragment") {
        ...
        checkAll(PropTestConfig(iterations = 50), Arb.graphQLOutputValue(schema, union, selections, config = config)) { value ->
            value.shouldBeInstanceOf<GeneratedObjectValue>().typeName.shouldBe("A")
        }
    }

    test("custom scalar values use ScalarValueOverrides") {
        ...
        checkAll(PropTestConfig(iterations = 20), Arb.graphQLOutputValue(schema, scalar, null, config = config)) {
