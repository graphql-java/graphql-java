...
object TypeTypeWeights : ConfigKey<Map<TypeType, Double>>(emptyMap(), Unvalidated) {
    ...
    val zero: Map<TypeType, Double> = TypeType.entries.associateWith { 0.0 }
}
...
/**
 * The probability that a `__typename` field value will be generated for an arbitrary object
 * value. This weight is sampled independently of [ImplicitNullValueWeight].
 *
 * For example, configuring [TypenameValueWeight] to 1.0 and [ImplicitNullValueWeight] to 1.0
 * causes a `__typename` field to be generated for every object value.
 */
...
/** Minimum query text length accepted by [graphQLTestData]. */
object MinQueryLength : ConfigKey<Int>(0, IntValidator(0..Int.MAX_VALUE))

/** Minimum number of external variables accepted by [graphQLTestData]. */
object MinVariablesCount : ConfigKey<Int>(0, IntValidator(0..Int.MAX_VALUE))
